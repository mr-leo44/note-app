                ...
                <form method="POST" action="{{ route('departments.store') }}">
                    ...
                    <div class="mb-4">
                        <label for="short_name"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Abréviation</label>
                        <input type="text" name="short_name" id="short_name" value="{{ old('short_name') }}"
                            autocomplete="off"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full p-2.5 @error('short_name') border-red-500 @enderror"
                            required>
                        @error('short_name')
                            <span class="text-red-600 text-xs">{{ $message }}</span>
                        @enderror
