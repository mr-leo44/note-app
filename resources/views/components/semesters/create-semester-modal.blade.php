<div id="createSemesterModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full flex justify-center items-center">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide="createSemesterModal">
                ...
                <form method="POST" action="{{ route('semesters.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full p-2.5 pr-10 appearance-none required">
                            ...
                            @foreach(App\Enums\ResultSemester::cases() as $semester)
                                <option value="{{ $semester->value }}" :selected="semester == {{ $semester->label() }}">{{ $semester->label() }}</option>
                            @endforeach
                        </select>
